...
class MP3:
    ...
    def __init__(self):
        ...
        # Track playing
        self.scale_of_track = ttk.Scale(self.root, from_=0, to=0, orient='horizontal', length=225,
                                        style='TScale',
                                        command=self.track_playing)
        self.scale_of_track.place(relx=0.5, rely=0.65, anchor=CENTER)
    ...
    def load(self):
        ...
        if len(self.list_of_songs_names) == 1:
            ...
            try:
                pygame.mixer.Sound(track_name)
                self.key_3 = self.list_of_songs[2]
                self.key_3_name = self.list_of_songs_names[2]
    ...
    def replay_playlist(self):  # NOT WORKING
        self.start_or_pause_btn.config(image=self.pause_png, command=self.pause)
    ...
    def replay_off(self):
        try:
            pygame.mixer.music.load(self.list_of_songs[0])
            pygame.mixer.music.play(2, start=self.val / 1000)
            pygame.mixer.music.set_endevent(0)
        except:
            pass
        else:
    ...
    def delete_third_from_playlist(self):
        try:
            if len(self.list_of_songs) == 1:
                ...
                del self.list_of_songs[(self.list_of_songs[::-1]).index(self.key_3)]
                del self.list_of_songs_names[(self.list_of_songs_names[::-1]).index(self.key_3_name)]
                self.btn_third.place_forget()
                self.btn_third_cancel.place_forget()
                self.track_name_on_screen.configure(text='')
                self.window.update_idletasks()
